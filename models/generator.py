...
class Generator(BaseModel):
    def define_model(self, model_name):
        ...
        x = Conv2DTranspose(filters=16, kernel_size=3, strides=2, output_padding=1,
                        use_bias=False, name=f'{model_name}_deconv_1',)(_input)
        x = BatchNormalization()(x)
        x = ReLU()(x)
        x = Conv2DTranspose(filters=8, kernel_size=5, strides=2, output_padding=1,
                        use_bias=False, name=f'{model_name}_deconv_2')(x)
        x = BatchNormalization()(x)
        x = ReLU()(x)
        x = Conv2DTranspose(filters=1, kernel_size=5, strides=2, output_padding=1,
